defmodule Web.AccountSessionController do
  use Web, :controller
  ...
  defp create(conn, %{"account" => %{"token" => token} = account_params}, info) do
    case Identity.login_account_by_magic_link(token) do
  ...
  defp create(conn, %{"account" => account_params}, info) do
    ...
    if account = Identity.get_account_by_email_and_password(email, password) do
      conn
      |> put_flash(:info, info)
      |> AccountAuth.log_in_account(account, account_params)
    else
