...
- **Multi pipelines should read like a story.** Use action-verb step names (`:added_photo`, `:removed_gallery`, `:authorized_comment`, `:updated_comment`) and matching action-verb function names (`add_photo_to_gallery/2`, `remove_gallery/2`). Avoid generic verbs like `:load` / `:take_loaded` / `:check` — they describe mechanics, not the story.
