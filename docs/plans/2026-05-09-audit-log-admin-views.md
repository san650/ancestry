...
All three LiveViews use `Permit.Phoenix.LiveView` with `resource_module: Ancestry.Audit.Log`. Unauthorized accounts redirect to `/org` with the standard flash, matching the pattern in `Web.AccountManagementLive.Index`.
...
This is a non-transactional side effect. It runs after `Repo.transaction/1` returns `{:ok, _}`, alongside the existing `:effects` firing — never inside the Multi. Failed dispatches (validation, authorization, conflict, exception) write nothing and broadcast nothing.
...
**Filter bar.** Two `<.input type="select">` controls on the top-level page; one (account) on the org-scoped page. Options for "Organization" come from `Ancestry.Organizations.list_organizations/0`. Options for "Account" come from a dedicated `Ancestry.Audit.list_audit_accounts/1` query that returns `DISTINCT account_id, account_email` from `audit_log` (optionally scoped to an organization), so the dropdown only shows accounts that have actually appeared in the log.
...
- `lib/web/components/nav_drawer.ex` (or wherever the drawer entries live) — two new conditional links.
